...
出力できるかを競うコンペ。AI Agentsの基礎となる技術に関わるコンペである。<br>
目次<br>
1. コンペ概要
2. 私の順位・スコア
3. メインコンペ取組み<br>
...
```bash
 ...
 "Text to YAML", 1%
```
※詳細は、"data/public_150.xlsx"を参照のこと。<br>
...
リーダボードでのモデルの実力確認ができない状況であったが、事務局から
提供されたGoogle Colabo実行環境では事前に正解率を以下の通り確認可能であった。<br>

|モデル|DBBench正解率|ALFWorld正解率|
|:-----|:-----|:-----|
|Qwen3-4B-Instruct-2507	|0.5368 |0.2600| 
|Qwen2.5-7B-Instruct	|0.5126 |0.5400|
...
ALFWorldの実行結果を解析し、(a)cool/warm処理+対象物の移動等や、(b)2物体処理の
ケースの正解率が低かった。データセットを解析することにより、これら(a),(b)の
データ増強を図った改善版のデータセットで実行したがスコア向上は見られなかった。
単純に同じデータを増やしてもエポック数を増やすことと同等と考える。やはり、
合成データにより(a),(b)のタスク範疇で似て非なる処理のデータを作成して
データ増強を行う必要があると考える。<br>
...
ハイパーパラメータや入力データセットによりスコアが大きく変動した。
SSLでさえパラメータ調整やスコアの再現性という観点でモデル開発は
難しい面があると感じた。ましてやLLMでは大量のデータ学習を伴うため
学習のエポック回数が数回と制限されるがどのように実行しているのか
興味が湧くところである。<br>

### 6.修了証
松尾研大規模言語モデル講座2025の基礎編及び応用編の修了証です。
講座を通してLLMに関する最新の知見を幅広く学ぶことができました。
今後この知見を業務に活用して行きたいと考えています。<br>

<img src="docs/images/certificate.jpg" alt="AI agents" width="480">
